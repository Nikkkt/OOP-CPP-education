						...
						if (actionChoice < 1 || actionChoice > 5) goto ACTIONMENU;
							...
							REPORTMENU: {
								...
								if (reportChoice == 1) {
									Date date = dayreport();
									ReportGenerator rg;
									std::map<Date, std::vector<Expense>> tmp = rg.generateDayReport(date, accounts);
									...
									ReportGenerator rg;
									std::map<Date, std::vector<Expense>> tmp = rg.generateWeekReport(date, accounts);
									int ReportAction = reportactions();
									if (ReportAction == 1) {
										Report report(tmp);
										report.ShowDayReport(rg, date, accounts);
									...
									ReportGenerator rg;
									std::map<Date, std::vector<Expense>> tmp = rg.generateMonthReport(date, accounts);
									int ReportAction = reportactions();
									if (ReportAction == 1) {
										Report report(tmp);
